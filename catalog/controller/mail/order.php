...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['products'] = array();
				...
				//products serials
        $auto_serial =  $this->config->get('module_serial_number_order_product_auto_serial');
        ...
        foreach ($order_products as $key => $value) {
            ...
            $product_serial = $this->model_extension_serial_number->getProductSerialsByOrderProduct($p_id);
            
            $product_serial_mode = $this->config->get('module_serial_number_product_serial_mode');

            if ($product_serial_mode == 2 || $product_serial_mode == 3) {
				//apply product serials
                if ($product_serial_mode == 2) {
                    if ($product_serial) {
                        foreach ($product_serial as $key0 => $value0) {
                            ...
                            if ($key0 < $p_qty) {
                                ...
                                $this->model_extension_serial_number->addOrderProductSerials($p_id, $serial);
				...
				//auto increment product serial
                if ($product_serial_mode == 3 && !empty($auto_serial)) {
                    for ($pro_serial=1; $pro_serial<=$p_qty; $pro_serial++) {
                        $serial = $auto_serial;
                        $this->model_extension_serial_number->addOrderProductSerials($p_id, $serial);
						array_push($string, $auto_serial);
        ...
        if(!empty($auto_serial)){
			$auto_serial = array('module_serial_number_order_product_auto_serial' => $auto_serial);
        	$this->model_setting_setting->editSetting('module_serial_number', $auto_serial);
        }
