	...
	protected function getList() {
		...
		if ($api_info && $this->user->hasPermission('modify', 'sale/order')) {
			...
			$data['api_token'] = '';
				...
				$data['add_serial_url'] = $this->url->link('extension/module/serial_number/applySerialNumbers', 'user_token=' . $this->session->data['user_token'], true);
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$products = $this->model_sale_order->getOrderProducts($this->request->get['order_id']);
				...
				$string = array();
				$p_id = $products[$key]['order_product_id'];


				$this->load->model('extension/serial_number');
				$products[$key]['serial'] = $this->model_extension_serial_number->getOrderProductSerials($p_id);
			
					if($products[$key]['serial']){

						$serial = $products[$key]['serial'];

						foreach ($products[$key]['serial'] as $key0 => $value0) {

							array_push($string, $value0['serial']);
						}

						$products[$key]['serial'] = implode("</br>", $string);
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				// get order products serials for invoice
				foreach ($products as $key => $value) {
	...
	public function shipping() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info && $order_info['shipping_code']) {
				...
				// get order products serials for shipping
				foreach ($products as $key => $value) {
